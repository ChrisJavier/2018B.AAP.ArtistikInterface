...
namespace Aplicaciones_En_Ambientes_Porpietarios
{
    public partial class Turismo : Form
    {
        ...
        {
            string eliminar = "exec dbo.eliminarRuta @NOMBRE='"+txtNombreP.Text+"'";
            if (txtNombreP.Text.Equals(""))
            {
                MessageBox.Show("No a escojido la ruta a eliminar");
            }
            else
            {
        ...
        {
            string consultarNombreR = bd.selectstring("select NOMBRE from RECORRIDO where NOMBRE = @NOMBRE");
            string registrarRecorrido= "exec dbo.registrarRecorrido "+
                "@CODGUIA = null,"+
                "@NOMBRE = '"+txtNombreR.Text+"',"+
                "@PRECIO = "+txtPrecioR.Text+" ,"+
                "@HINICIO = '"+txtHoraI.Text+"'," +
                "@HFIN = '"+txtHoraF.Text+"' ,"+
                "@FECHA = '"+dtpFechaR.Text+"'";
            if (txtNombreR.Text.Equals("") || txtPrecioR.Text.Equals("") || txtHoraI.Text.Equals("") || txtHoraF.Text.Equals("") || dtpFechaR.Text.Equals(""))
            {
                MessageBox.Show("Error uno o mas campos vacios");
            }
            else
            {
                if (consultarNombreR == txtNombreR.Text)
                {
                    MessageBox.Show("Datos ya registrados");
                }
                else
                {
                    if (bd.executecommand(registrarRecorrido))
                    {
        ...
        {
            string consultarCodR = bd.selectstring("exec dbo.buscarCodigoRecorrido @NOMBRE='"+txtNombreR.Text+"'");
            string consultarNombreP = bd.selectstring("SELECT NOMBREPUNTO FROM RUTA WHERE NOMBREPUNTO='"+txtNombreP.Text+"'");
            int codRecorrido = Int32.Parse(consultarCodR);
            string longitud = txtLongitud.Text;
            string[] lon = longitud.Split(',');
            string lonFinal = lon[0] + '.' + lon[1];
            string latitud = txtLatitud.Text;
            string[] lat = latitud.Split(',');
            string latFinal = lat[0] + '.' + lat[1];
            string registrarRuta = "exec dbo.registrarRuta " +
                "@CODRECORRIDO = "+codRecorrido+"," +
                "@NOMBRE = '" + txtNombreP.Text + "'," +
                "@DESCRIPCION = " + txtDescripcion.Text + " ," +
                "@LONGITUD = " + lonFinal + "," +
                "@LATITUD = " + latFinal + " ," +
                "@NUMPARADA = " + txtNumParada.Text + "";
            if (txtNombreP.Text.Equals("") || txtDescripcion.Text.Equals("") || txtLongitud.Text.Equals("") || txtLatitud.Text.Equals("") || txtNumParada.Text.Equals(""))
            {
                MessageBox.Show("Error uno o mas campos vacios");
            }
            else
            {
                if (consultarNombreP == txtNombreP.Text)
                {
                    MessageBox.Show("Datos ya registrados");
                }
                else
                {
                    if (bd.executecommand(registrarRuta))
                    {
                        MessageBox.Show("Registrado");
                        txtNombreP.Text = "";
                        txtDescripcion.Text = "";
        ...
        private void txtPrecioR_KeyPress(object sender, KeyPressEventArgs e)
        {
